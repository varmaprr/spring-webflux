package com.varma.springflux.springfluxdemo.controller;

import com.varma.springflux.springfluxdemo.model.Product;
import com.varma.springflux.springfluxdemo.model.ProductEvent;
...
public class ProductController {
    ...
    public Mono<ResponseEntity<Product>> getProductById(@PathVariable String id) {
        return repository.findById(id).map(product -> ResponseEntity.ok(product)).defaultIfEmpty(ResponseEntity.notFound().build());
    }
    ...
    public Mono<ResponseEntity<Void>> deleteProduct(@PathVariable(value="id") String id){
        return repository.findById(id).flatMap(existingProduct -> repository.delete(existingProduct)
